...
const Profile = () => {
    ...
    useEffect(() => {
        fetchData().then((data: Customer[]) => { setCustomers(getCustomers(data)); setLoading(false); });
    }, [fetchData]);
    ...
    const countryBodyTemplate = (rowData: Customer) => {
        return (
            <div className="flex align-items-center gap-2">
                <img alt="flag" src={`https://flagsapi.com/${rowData.country.code.toUpperCase()}/flat/32.png`} className={`flag flag-${rowData.country.code}`} style={{ width: '24px' }} />
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            <div className="h-200 max-w-full overflow-x-auto">          
                <div className="card p-fluid">
                    <DataTable  value={customers} paginator showGridlines rows={10} dataKey="id" filters={filters} filterDisplay="row" loading={loading} globalFilterFields={['name', 'country.name', 'representative.name', 'status']} header={header} emptyMessage="No customers found.">
